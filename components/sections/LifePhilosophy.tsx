...
const philosophies = [
  ...
  },
  {
    ...
    author: "Prem Mandal", // Ya Anonymous jo bhi rakhna ho
          ...
          {philosophies.map((item, index) => (
            <motion.div
              ...
              className={`relative p-6 sm:p-8 rounded-3xl backdrop-blur-xl border transition-all duration-300 flex flex-col justify-between group hover:-translate-y-2 ${
                item.highlight
                  ? "bg-gradient-to-b from-blue-600/15 via-white/[0.03] to-white/[0.02] border-blue-500/40 shadow-[0_0_40px_rgba(59,130,246,0.15)]"
                  : "bg-white/[0.03] border-white/10 hover:border-blue-500/30 hover:shadow-2xl"
              }`}
            >
              <div>
                {/* Quote Icon */}
                <div className="text-blue-400/80 mb-4 group-hover:scale-110 transition-transform">
                  <Quote className="w-8 h-8 fill-blue-500/10" />
                </div>

                {/* Quote Text */}
                <p className="text-gray-200 text-sm sm:text-base leading-relaxed font-medium italic">
                  &quot;{item.quote}&quot;
                </p>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between">
                ...
                <span className="w-2 h-2 rounded-full bg-blue-500/50 group-hover:bg-cyan-400 transition-colors" />
              </div>
            </motion.div>
          ))}
        </div>
