...
export default function CommunityPresence() {
  return (
    <section className="py-24 relative overflow-hidden bg-background text-white border-t border-white/5">
      ...
      <Container className="relative z-10">
        ...
        >
          ...
          <p className="mt-4 text-xs sm:text-base text-gray-400 max-w-2xl mx-auto font-medium leading-relaxed">
          ...
          {communityPlatforms.map((platform, index) => {
            ...
            return (
              <motion.a
                ...
                className={`group relative p-5 sm:p-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl transition-all duration-500 hover:-translate-y-1.5 ${platform.borderColor} ${platform.glowColor} flex flex-col items-center justify-between text-center`}
              >
                {/* Platform Icon Container */}
                <div className="p-3 sm:p-4 rounded-xl bg-white/5 border border-white/10 text-cyan-400 group-hover:scale-110 group-hover:bg-blue-500/10 group-hover:text-white transition-all duration-300">
                  {platform.isBrand ? (
                    <Icon size={26} />
                  ) : (
